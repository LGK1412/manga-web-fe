...
export default function CreateStoryPage() {
    ...
    useEffect(() => {
        let mounted = true
            ; (async () => {
                try {
                    const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/genre`, { withCredentials: true })
    ...
    const handlePublish = async () => {
        ...
        const payload = { ...v, status: (v.status || "").toLowerCase(), type: storyType, isDraft: false }
        ...
        try {
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/manga/${authorId}`, payload, { withCredentials: true })
    ...
    const getCurrentStoryChapters = () => {
        ...
        return chapters.filter((chapter) => chapter.storyId === currentStoryId && chapter.storyType === storyType)
    }
                            ...
                            <CardContent className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            storyType === "text" ? setTextStoryTitle(e.target.value) : setImageStoryTitle(e.target.value)
                                        }
                                    />
                                </div>
                                                ...
                                                <div key={g._id} className="flex items-center space-x-2">
                                                    <Checkbox
                                                        ...
                                                        onCheckedChange={(c) => {
                                                            ...
                                                            storyType === "text" ? setTextSelectedGenres(next) : setImageSelectedGenres(next)
                                                        }}
                                                    />
