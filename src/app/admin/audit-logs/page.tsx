...
export default function AuditLogsPage() {
  ...
  const handleMarkSeen = async (logId: string) => {
    ...
    try {
      ...
      await axios.patch(endpoint, {}, { withCredentials: true });
      setLogs((prev) =>
        prev.map((l) => (l.id === logId ? { ...l, seen: true } : l)),
      );
    } catch (err: any) {
      console.error(
        "[AuditLogs] markSeen error",
        err?.response?.data || err?.message,
      );
    } finally {
      setLoading(false);
      ...
      <div className="space-y-6">
        ...
        {!isAdmin && (
          <p className="text-xs text-gray-500">
            You are logged in as <b>{prettyRole(me?.role ?? "unknown")}</b>.
            Approve/Seen actions are admin-only.
          </p>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
              ...
              >
                ...
                <SelectContent>
                  ...
                  {/* report */}
                  <SelectItem value="report_status_new">
                    Report · Status set to New
                  </SelectItem>
                  <SelectItem value="report_status_in-progress">
                    Report · Status set to In Progress
                  </SelectItem>
                  <SelectItem value="report_status_resolved">
                    Report · Resolved
                  </SelectItem>
                  <SelectItem value="report_status_rejected">
                    Report · Rejected
                  </SelectItem>
                  <SelectItem value="report_update">
                    Report · Updated
                  </SelectItem>

                  {/* comment/reply */}
                  <SelectItem value="comment_hidden">
                    Comment · Hidden
                  </SelectItem>
                  <SelectItem value="comment_restored">
                    Comment · Restored
                  </SelectItem>
                  <SelectItem value="reply_hidden">Reply · Hidden</SelectItem>
                  <SelectItem value="reply_restored">
                    Reply · Restored
                  </SelectItem>
                </SelectContent>
              </Select>
                  ...
                  {loading ? (
                    ...
                    logs.map((log) => (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            className={`text-xs ${getRiskColor(log.risk)}`}
                          >
                            {log.risk === "high" && (
                        ...
                        <TableCell className="text-right space-x-1">
                          <Button
                            ...
                            onClick={() =>
                              router.push(
                                `/admin/audit-logs/log-details/${log.id}`,
                              )
                            }
                          >
