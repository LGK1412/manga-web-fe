...
type AuditLog = {
  ...
  actor: {
    name: string
    email: string
    role: "content_moderator" | "community_manager" | "system"
  }
  action: string
  target: {
    type: "Report" | "Manga" | "Chapter" | "Comment" | "Reply"
    id: string
    reportCode?: string
  }
  summary: string
...
const mockLogs: AuditLog[] = [
  {
    ...
    actor: {
      name: "Sarah Chen",
      email: "sarah.chen@mangawords.com",
    ...
    target: {
      type: "Report",
      id: "RPT-2025-001",
      reportCode: "VIOLENCE_HIGH",
    },
    summary: "Rejected report for manga chapter containing excessive violence",
    risk: "high",
    seen: false,
    approval: "pending",
    before: { status: "pending", reason: "Graphic violence detected" },
    after: { status: "rejected", reviewed_at: "2025-01-24 14:32:15" },
    note: "False positive - content is within community guidelines",
  }
]
...
export default function AuditLogsPage() {
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      const matchesSearch =
        ...
        log.target.reportCode?.toLowerCase().includes(search.toLowerCase()) ||
        log.target.id.toLowerCase().includes(search.toLowerCase()) ||
        log.summary.toLowerCase().includes(search.toLowerCase())

      const matchesRole =
        roleFilter === "all" ||
        (roleFilter === "system" && log.actor.role === "system") ||
        (roleFilter === "content_moderator" && log.actor.role === "content_moderator") ||
        (roleFilter === "community_manager" && log.actor.role === "community_manager")

      const matchesAction = actionFilter === "all" || log.action === actionFilter

      const matchesStatus =
        statusFilter === "all" ||
        (statusFilter === "unseen" && !log.seen) ||
        (statusFilter === "seen" && log.seen) ||
        (statusFilter === "pending" && log.approval === "pending") ||
        (statusFilter === "approved" && log.approval === "approved")

      const matchesRisk = !highRiskOnly || log.risk === "high"
  ...
  }, [logs, search, roleFilter, actionFilter, statusFilter, highRiskOnly])
  ...
  const handleMarkAllSeen = () => {
    setLogs(logs.map((log) => ({ ...log, seen: true })))
  }

  const handleMarkSeen = (logId: string) => {
    setLogs(logs.map((log) => (log.id === logId ? { ...log, seen: true } : log)))
    setSelectedLog((prev) =>
      prev?.id === logId ? { ...prev, seen: true } : prev
    )
  }
  ...
  const handleApprove = (logId: string) => {
    setLogs(
      logs.map((log) =>
        log.id === logId ? { ...log, approval: "approved" } : log
      )
    )
    setSelectedLog((prev) =>
      prev?.id === logId ? { ...prev, approval: "approved" } : prev
    )
    setAdminNote("")
  }
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-4 flex-col lg:flex-row">
            <div className="flex-1">
              <Input
                placeholder="Search by actor, email, report code, content id…"
                value={search}
                onChange={(e) => {
                ...
                {paginatedLogs.map((log) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge
                        variant={
                          log.approval === "approved" ? "default" : "outline"
                        }
                        className={`text-xs ${
          ...
          <SheetContent className="w-full sm:w-[500px] overflow-y-auto">
            ...
            <div className="space-y-6 mt-6">
              ...
              <div>
                ...
                <Badge
                  className={`mt-2 ${getRoleColor(selectedLog.actor.role)}`}
                >
