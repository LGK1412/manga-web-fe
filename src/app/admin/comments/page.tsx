...
// [31/10/2025] Hằng số “All …” dùng chuỗi rỗng trong state (Radix Select xử lý qua sentinel ở component filters)
const ALL = {
  MANGA: "",
  CHAPTER: "",
  STATUS: "",
...
export default function CommentsPage() {
  ...
  // [31/10/2025] Tải comment + manga. KHÔNG còn gọi /api/chapter ở đây nữa
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const [commentRes, mangaRes] = await Promise.all([
          axios.get(`${API}/api/comment/all`, { withCredentials: true, signal: controller.signal }),
          axios.get(`${API}/api/manga`, { withCredentials: true, signal: controller.signal }),
        ]);

        // Map comments -> UI type
  ...
  // [31/10/2025] Khi chọn Manga → gọi BE /api/chapter/by-manga/:id để lấy chapter
  useEffect(() => {
    const controller = new AbortController();
    (async () => {
      if (!API) return;
      // Chưa chọn manga ⇒ clear chapters
      ...
      try {
        ...
        setChapters([]); // fallback rỗng để Select không crash
      }
    })();
    return () => controller.abort();
  }, [API, filters.manga]);
  ...
  // Toggle visibility — đúng route BE: /api/comment/toggle/:id
  const handleToggleVisibility = async (id: string, currentStatus: string) => {
    try {
      ...
      setComments((prev) =>
        prev.map((c) => (c.id === id ? { ...c, status: currentStatus === "visible" ? "hidden" : "visible" } : c))
      );
